...
app.post("/api/items", auth, (req, res) => {
  const { name, description="", category, condition, exchangeType, location, image="", preferredExchange="" } = req.body;
  if (!name || !category || !condition || !exchangeType || !location) {
    return res.status(400).json({ message: "Please fill all required item fields." });
  }
  const result = db.prepare(`
    INSERT INTO items(user_id,name,description,category,condition,exchange_type,location,image,preferred_exchange)
    VALUES(?,?,?,?,?,?,?,?,?)
  `).run(req.user.id, name, description, category, condition, exchangeType, location, image, preferredExchange);
